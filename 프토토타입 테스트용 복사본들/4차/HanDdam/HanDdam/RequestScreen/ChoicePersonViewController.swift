...
class ChoicePersonViewController: UIViewController {
    ...
    @IBAction func nextBtnClicked(_ sender: Any) {
        ...
        //request 하나만 생성을 시켜놓아도 알아서 detailRequest가 들어가게 되니까 굳이 detailRequest를 따로 생성시킬 필요가 없다.
        //여기 request를 segue로 다음 VC으로 전달시키면 된다.
        self.request.detailRequests = people.map { person in
            let detailRequest = DetailRequest()
            detailRequest.person = person
            return detailRequest
        }
        //여기서 request.detailRequests가 제대로 들어가 있는 것 확인했음.
        print(self.request.detailRequests)
    }
