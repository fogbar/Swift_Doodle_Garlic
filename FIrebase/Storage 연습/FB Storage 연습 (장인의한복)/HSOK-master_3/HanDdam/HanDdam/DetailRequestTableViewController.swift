...
extension DetailRequestTableViewController : UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        if let selectedImage = selectedImageFromPicker {
            ...
            let indexPath = IndexPath(row: 0, section: 4)
            guard let itemCell = tableView.cellForRow(at: indexPath) as? ImageCell else {return}
            itemCell.hanbokImage.image = selectedImage
            request.detailRequests[self.selectedIndex].detailImage = selectedImage
        }
        
        //        if let image = info[.editedImage] as? UIImage, let imageUrl = info[UIImagePickerController.InfoKey.referenceURL] as? URL{
        //            //현재 containerVC에 이미지가 보여지도록 함
        //            guard let containerVC = self.parent as? ContainerViewController else {return}
        //            guard let request = containerVC.request else {return}
        //
        //            let indexPath = IndexPath(row: 0, section: 4)
        ...
        ////            let filePath = FBStorageService.instance.requestImagesRef.child(imageName)
        ////            let metaData = StorageMetadata()
        ////            metaData.contentType = "image/jpeg"
        ////
        ////            filePath.putFile(from: imageUrl, metadata: metaData) { (mData, err) in
        ////                if let err = err {
        ////                    print("Err : \(err)")
        ////                } else {
        ////                    let downloadURL = mData!.downloadURL()!.absoluteString
        ////                    //이제 이걸 가지고 AFImage를 통해 보여주도록 하면 됨.
        ////                }
        ////            }
        //
        //        }
